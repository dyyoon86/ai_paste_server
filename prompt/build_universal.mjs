// skill/SKILL.md → prompt/UNIVERSAL_PROMPT.md 생성.
...
let md = readFileSync(path.join(ROOT, "skill", "SKILL.md"), "utf8");
...
md = md
  .replace(/# Remotion Shorts Spec \(PasteMotion 두뇌\)\s*/, "")
  .replace(
    /너는 숏폼 영상 \*\*기획 두뇌\*\*다\. 무료 Claude 환경에서는 영상을 직접 만들 수 없으므로, 너의 유일한 산출물은[\s\S]*?MP4로 렌더한다\./,
...
writeFileSync(path.join(DIR, "UNIVERSAL_PROMPT.md"), header + md.trim() + footer);
console.log("생성됨: prompt/UNIVERSAL_PROMPT.md");
